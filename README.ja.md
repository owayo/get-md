...
- **CommonMark準拠のフェンス判定** — 開始・終了フェンスは外側のリスト項目の内容インデント基準で最大3スペースまでを許容し、それを超えるバッククォート行はインデントコードとして保持する。リストマーカーと同じ行から始まるフェンスも認識し、リストやブロッククォートの終了時にフェンス状態を正しく閉じる。info string 付きの行（例: ` ```rust `）も閉じフェンスとして扱わない。テーブル圧縮も同じブロック分類を再利用するため、これらのフェンス内にあるテーブル風コードを保持し、フェンス終了後に圧縮を再開する
...
- **段落境界を守るリンク解析** — 未閉鎖の `[` やインラインバッククォートを、ブロッククォート記号だけの空行を含む段落境界・コードブロック境界の先にある区切りと結合せず、単一の soft break を含む正規のリンクテキストは維持する。壊れたリンクの閉じ区切り探索も、ブロッククォート記号だけの空行で打ち切る
- **未閉鎖バッククォート対応** — 閉じていないインラインバッククォートはリテラルとして扱い、その後ろの Markdown リンク解決を妨げない。閉じバッククォートの先読みは、リストマーカーと同じ行から始まるフェンスでも打ち切る
- **線形時間のバッククォート照合** — 段落・フェンス境界内の同じ長さのバッククォート列を事前に索引化し、未閉鎖の列が多数ある文書でも後方の繰り返し走査を防ぐ
