...
- E2E テストは実際の Chrome/Chromium が必要（`#[ignore]` 付き）。GitHub Raw の実取得、ローカル `file://` ページでの相対 URL 解決、複数セレクタの `---` 結合、`--ignore-date` の書き込み抑止を確認する。一時ディレクトリは時刻・プロセス ID・プロセス内連番で一意化し、並列実行時の衝突を防ぐ
